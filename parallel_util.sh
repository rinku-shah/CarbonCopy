...
dg1=`sudo lxc-info -p -n gateway|tr -d ' '|cut -d ':' -f2`
...
sg11=`sudo lxc-info -p -n primary3|tr -d ' '|cut -d ':' -f2`
sg12=`sudo lxc-info -p -n secondary3|tr -d ' '|cut -d ':' -f2`
...
ran1=`sudo lxc-info -p -n load_gen|tr -d ' '|cut -d ':' -f2`
...
sh cpu_util.sh $1 $2 $dg1 gateway.txt tmp_d1 $3 "^%Cpu10 \|^%Cpu11 " &
...
sh cpu_util.sh $1 $2 $sg11 primary.txt tmp_s11 $3 "^%Cpu12 \|^%Cpu13 " &
sh cpu_util.sh $1 $2 $sg12 secondary.txt tmp_s12 $3 "^%Cpu14 \|^%Cpu15 " &
