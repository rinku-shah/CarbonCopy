...
int Network::ProcessPacket2(unsigned char* buffer, int size)
{   
 ...
 void Network::print_ip_header2(unsigned char* Buffer, int Size)
{
...
void Network::print_udp_packet2(unsigned char *Buffer , int Size)
{
    unsigned short iphdrlen;
    struct iphdr *iph = (struct iphdr *)(Buffer +  sizeof(struct ethhdr));
    iphdrlen = iph->ihl*4;
